...
### 열쇠는 탑 전체에서 모자라다 (魔塔 분석)

원형의 50층은 문:열쇠가 1:1 이 아니다 — 노랑 239:169(71%), 파랑 39:20(51%),
빨강 11:3(27%). **색이 귀할수록 결핍이 가파르다.** 그래야 "어느 문을 열까" 가
질문이 된다.

우리는 층마다 문 셋이 구역을 잘라 순서를 강제하므로 **큰길 열쇠는 줄일 수 없다**
— 하나만 빼도 진행이 막힌다. 그래서 줄이지 않고 늘려서 모자라게 한다. 둘 다
골방(`_carve_alcove`) 위에 얹으므로 **완주 계산은 한 칸도 달라지지 않는다.**

| 골방 | 내용 | 수 |
|---|---|---|
| 여분 열쇠 | 파수꾼 뒤에 열쇠 하나 더. 안 쓰면 다음 층으로 들고 간다 | 초록 12 · 노랑 8 · 빨강 4 |
| 금고 | 골방 입구가 **네 번째 문**. 안에 룬이 있다 | 초록 20 · 노랑 16 · 빨강 12 |

배분은 `generate_content.py` 의 `_ALCOVE_CYCLE` 한 곳에서만 정한다(24층 한 바퀴 x 4).

**금고는 마지막 구역에만 둔다.** 앞 구역에 두면 그 층에서 방금 주운 열쇠로
열리고, 그러면 큰길 문을 열 열쇠가 사라져 **되돌릴 수 없이 갇힌다**(되돌아가도
아랫층은 이미 비어 있다). 세 문을 다 연 뒤라야 손에 남은 것이 진짜 여분이다.
`build_floor_layout` 이 그때만 `order[7:]` 에서 골방을 파고,
`layout_gen.check_vault_safe()` 가 완성된 격자에서 다시 잰다 — 0건이어야 한다.

금고마다 **다른 룬**을 넣는다(`RUNE_CYCLE` 을 금고 순번으로 돈다). 같은 것만
나오면 "지금 열까 아껴 둘까" 가 아니라 "빨리 열수록 이득" 뿐이라 고를 것이 없다
— 물약에서 이미 겪은 일이다.

`ContentValidator.cs` 는 층마다 문 수 != 열쇠 수를 **경고**로 찍는다. 지금은
그게 설계이므로 그 경고는 무시한다. 하드 게이트는 `generate_content` 의 `[6/7]`
과 `validate_content.py` 의 층별 검사다.
...
**셀 코드와 프리팹 이름은 순서가 다르다.** 셀 코드는 방향이 먼저지만
(3/4/5 가로, 6/7/8 세로), 프리팹은 **색이 먼저고 방향이 나중**이다 —
`Tilemap_3/4` 초록, `5/6` 노랑, `7/8` 빨강이고 짝의 뒤쪽(4·6·8)만 Y 90° 로 돌아 있다.
회전은 프리팹에 구워져 있고 `BuildDoor` 는 회전을 주지 않으므로, 셀 코드를 그대로
프리팹 이름으로 쓰면 그림이 통째로 90° 어긋난다 (매 층 두 번째 문 96개가 그랬다).
번역은 `BuildDoor` 의 `artId` 와 `layout_gen.door_art` 두 곳뿐이고 서로 같아야 한다.

방향이 자리와 어긋나면 벽이 없는 쪽으로 문틀이 떠 있게 되고, 옆으로 돌아갈 수
있는 것처럼 보인다. 그래서 **통로 방향이 문 방향을 정한다** — 좌우로 뻗은 통로에는
세로문, 위아래로 뻗은 통로에는 가로문.

`layout_gen.check_doors()` 가 문마다 **자리·그림·색**을 다 본다 — 벽 규칙뿐 아니라
실제로 놓일 프리팹을 디스크에서 읽어 방향과 색을 대조한다. 표를 파이썬에 또 적어 두면
두 벌이 되어 같은 방식으로 다시 어긋나기 때문이다.

```bash
python Tools/layout_gen.py    # 100층 CSV 를 전부 훑는다. 위반 0 이어야 한다
```

`generate_content.py` 의 `[4/5]` 단계도 같은 검사를 돌린다. 여기서 걸리면 씨앗을
다시 뽑아도 소용없다 — 규칙이 아니라 번역표가 어긋난 것이다.
...
(관문인 보스 0/5), 룬도 96개 층 중 94개에서 그냥 지나칠 수 있었다. 그런데
완주 시뮬레이션은 룬을 늘 주운 것으로 셈하고 있었으니 "완주 보장" 이 그만큼 거짓이었다.

그래서 **계단이 있는 마지막 방으로는 곁길을 내지 않는다.** 입구가 하나뿐이라야
