...
에디터가 프로젝트를 잠그므로 **에디터를 닫고** 명령줄에서 두 번 부른다.
...
에디터 MCP(`unity-editor-mcp`)는 명령을 **메인 스레드에서** 처리한다. 오래 걸리는
호출(`BuildPlayerContent` 같은)을 `eval` 로 던지면 그 뒤 모든 명령이 60초 타임아웃으로
죽는다. 이때 프로세스는 `Responding=True` 이고 CPU 는 유휴다 — 즉 **일하는 중이 아니라
파이프라인만 물린 것**이다. `CloseMainWindow` 도 안 먹으면 강제 종료 후
`python Tools/clean_editor_state.py` 로 잠금 파일을 치운다.

긴 작업은 MCP `eval` 이 아니라 위의 명령줄 `-executeMethod` 로 돌린다.

### 층에는 같은 종이 셋 선다

다섯 자리가 전부 다른 종이면 **"이 놈에겐 공격 1점이 몇 대 값이다" 를 셀 이유가 없다.**
한 번 싸우고 마는 상대의 값은 계산할 까닭이 없기 때문이다. 그래서 앞 세 자리를 한
종으로 묶는다(`MOB_SPECIES_RUN = [3, 1, 1]`) — 같은 놈을 세 번 만나니 공격 +1 이 세 번
값을 하고, 거기서 "몇 점을 올리면 한 대가 준다" 는 임계가 생긴다.
(원형인 매직 타워도 한 종을 평균 2.85 마리씩 세운다.)

**묶인 자리는 스탯·특성·그림이 전부 같아야 한다** — 하나라도 다르면 같은 놈이 아니다.
`solve_monster` 은 묶음의 첫 자리로 한 번만 돌린다. 손실 배수
`[0.85, 0.85, 0.85, 1.05, 1.50]` 의 총합 5.10 은 예전 값과 같아서 층당 1레벨은 그대로다.
...
바닥까지 내려가 **HP 1 짜리가 40마리** 나왔다. 하한을 두고 손실 목표는 ATK 탐색에
맡긴다.

### 챕터 조명은 Effects_00 이 덮어쓸 수 있다

챕터가 "색 필터를 씌운 것" 으로만 보였던 진짜 이유는 조명이 **아예 적용되지 않아서**였다.
`GenerateMap` 이 `ChapterTheme.Apply` 로 해의 색·각도·세기를 정한 직후,
`Effects_00.Start` 가 `OnPortalAction` 을 쏘고 그 `SetLight` 가 색을 하드코딩 값으로
덮어썼다. 안개만 바뀌고 그림자 방향은 100층 내내 같았다.

생성 층은 `ChapterTheme` 이 정한 시간대를 그대로 쓴다. 손수 만든 1~4층만 예전 값을
남긴다 — 그 층 조명은 인트로 연출에 맞춰져 있다.

**공중에 떠도는 것도 챕터가 정한다.** 벽 아트가 한 세트뿐이라, 방에 들어섰을 때
"여기는 다른 곳이다" 를 가장 먼저 말해 주는 것이 이 알갱이들이다. 같은 파티클을
색·중력·크기·배속·빈도만 바꿔 쓴다 — 잎(묘소) · 물방울(수로) · 불티(용광로, 중력이
음수라 떠오른다) · 눈(심층) · 티끌(균열).

BGM 은 곡이 챕터 00 것 하나뿐이라 **챕터마다 조·속도를 달리해** 쓴다
(`ChapterTheme.BgmPitch`). 새 곡이 들어오면 1 로 두고 어드레서블 키만 채우면 된다.
`FadeAndPlayBGM` 은 예전에 **이미 재생 중일 때만** 곡을 갈아서, 꺼져 있으면 그대로
무음이었다.
...
되돌아갔다** — 상대 방패를 부술수록 내 방패가 늦게 올라왔다. 파이썬 시뮬레이터는
그 동작을 모델링하지 않았으므로, **완주 검증이 보스전 피해를 최대 34% 적게 보고 있었다.**
부모는 자기 게이지만 지우고 플레이어 몫은 `UI_PlayerCard` 가 지운다.

대조하는 법 — 파이썬으로 값을 뽑고 에디터에서 같은 조건으로 예측을 돌린다.
에디터에는 어드레서블 선로드가 없어 표가 비어 있으므로 먼저 올린다:

```csharp
FastChecks.EditorBootData();          // 표를 올린다
